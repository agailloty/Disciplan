...
internal static class MappingExtensions
{
    ...
    internal static BoardSummaryDto ToSummaryDto(this Board board, string requestingUserId) => new(
        board.Id,
        board.Name,
        board.Description,
        board.Columns.Count,
        ...
        board.GetEffectiveRole(requestingUserId) ?? MemberRole.Guest,
        board.OwnerId == requestingUserId
    );

    internal static BoardDetailDto ToDetailDto(this Board board, string requestingUserId,
        IReadOnlyList<MemberDto> members) => new(
        board.Id,
        board.Name,
        board.Description,
        board.OwnerId,
    ...
    internal static ProjectSummaryDto ToSummaryDto(this Project p, int ticketCount, string requestingUserId) =>
        new(p.Id, p.Name, p.Description, p.Key, ticketCount, p.Sprints.Count, p.CreatedAt,
            p.GetEffectiveRole(requestingUserId) ?? MemberRole.Guest,
            p.OwnerId == requestingUserId);

    internal static ProjectDetailDto ToDetailDto(this Project p, IReadOnlyList<SprintDto> sprints,
        string requestingUserId, IReadOnlyList<MemberDto> members) =>
        new(p.Id, p.Name, p.Description, p.Key, p.OwnerId, p.CreatedAt, p.UpdatedAt,
            ...
            members, p.GetEffectiveRole(requestingUserId) ?? MemberRole.Guest);
}
